    ...
    OwnableUpgradeable
{
    ...
    mapping(address user => mapping(IERC4626 vault => uint256 shares))
        public collateralShares;
    ...
    function setCollateralizationRatio(uint256 newRatio) external onlyOwner {
        require(
            newRatio >= 10000,
            "CollateralManager: ratio must be at least 100%"
        );
        require(
            newRatio <= 30000,
            "CollateralManager: ratio must be at most 300%"
        );
    ...
    function setLiquidationBonus(uint256 newBonus) external onlyOwner {
        require(
            newBonus <= 2000,
            "CollateralManager: bonus must be at most 20%"
        );
    ...
    function addSupportedVault(IERC4626 vault) external onlyOwner {
        require(
            !isSupportedVault[vault],
            "CollateralManager: vault already supported"
        );
    ...
    function removeSupportedVault(IERC4626 vault) external onlyOwner {
        require(
            isSupportedVault[vault],
            "CollateralManager: vault not supported"
        );
    ...
    function depositCollateral(IERC4626 vault, uint256 shares) external {
        ...
        uint256 requiredCollateralForDebt = (currentDebt *
            collateralizationRatio) / RATIO_PRECISION;

        if (availableCollateralValue > requiredCollateralForDebt) {
            uint256 excessCollateral = availableCollateralValue -
                requiredCollateralForDebt;
            uint256 stablecoinToMint = (excessCollateral * RATIO_PRECISION) /
    ...
    function withdrawCollateral(IERC4626 vault, uint256 shares) external {
        ...
        require(
            collateralShares[msg.sender][vault] >= shares,
            "CollateralManager: insufficient collateral"
        );
        ...
        uint256 minRequiredCollateral = (currentDebt * collateralizationRatio) /
            RATIO_PRECISION;

        require(
            collateralValueAfter >= minRequiredCollateral,
    ...
    function repayDebt(uint256 amount) external {
        ...
        require(
            ...
            "CollateralManager: insufficient debt"
        );
    ...
    function liquidate(
        ...
        require(
            user != msg.sender,
            "CollateralManager: cannot liquidate own position"
        );
        require(
            isLiquidatable(user),
            "CollateralManager: position not liquidatable"
        );
        require(
            isSupportedVault[vault],
            "CollateralManager: vault not supported"
        ...
        require(
            collateralShares[user][vault] > 0,
            "CollateralManager: no collateral in this vault"
        );
        require(
            debtToRepay > 0,
            "CollateralManager: debt to repay must be greater than 0"
        );

        uint256 userDebtAmount = userDebt[user];
        uint256 actualDebtToRepay = debtToRepay > userDebtAmount
            ? userDebtAmount
            : debtToRepay;

        uint256 collateralValueToSeize = (actualDebtToRepay *
            (BONUS_PRECISION + liquidationBonus)) / BONUS_PRECISION;

        uint256 collateralSharesToSeize = vault.convertToShares(
        ...
        if (collateralSharesToSeize > userCollateralShares) {
            ...
            uint256 actualCollateralValue = vault.convertToAssets(
                collateralSharesToSeize
            );
            actualDebtToRepay =
                (actualCollateralValue * BONUS_PRECISION) /
                (BONUS_PRECISION + liquidationBonus);
        }

        require(
            actualDebtToRepay > 0,
            "CollateralManager: insufficient collateral for liquidation"
        );
        ...
        uint256 actualCollateralValueSeized = vault.convertToAssets(
            collateralSharesToSeize
        );
        uint256 bonus = actualCollateralValueSeized > actualDebtToRepay
            ? actualCollateralValueSeized - actualDebtToRepay
            : 0;
