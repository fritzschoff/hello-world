...
contract DeployScript is Script {
    ...
    function run() public {
        ...
        console.log(
            "HelloWorld implementation deployed at:",
            address(helloWorldImpl)
        );

        bytes memory helloWorldInitData = abi.encodeCall(
            HelloWorld.initialize,
            (deployer)
        );
        ERC1967Proxy helloWorldProxy = new ERC1967Proxy(
            address(helloWorldImpl),
            helloWorldInitData
        );
        ...
        console.log(
            "Stablecoin implementation deployed at:",
        ...
        console.log(
            "CollateralManager implementation deployed at:",
            address(collateralManagerImpl)
        );

        bytes memory collateralManagerInitData = abi.encodeCall(
            CollateralManager.initialize,
            (address(stablecoin), deployer)
        );
        ERC1967Proxy collateralManagerProxy = new ERC1967Proxy(
            address(collateralManagerImpl),
            collateralManagerInitData
        );
        collateralManager = CollateralManager(address(collateralManagerProxy));
        console.log(
            "CollateralManager proxy deployed at:",
        ...
        console.log(
            "Governor implementation deployed at:",
